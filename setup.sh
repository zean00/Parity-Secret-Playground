...
sed  -i  s,accountx,$acc0,g parity/config/authority0.toml
sed  -i  s,accountx,$acc1,g parity/config/authority1.toml
sed  -i  s,accountx,$acc2,g parity/config/authority2.toml

for i in `seq 0 2`;
do
sed -i  "/engine_signer/s/^#//g" parity/config/authority$i.toml
done
